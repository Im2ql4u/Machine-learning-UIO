...
def hartree_fock_2d(n_electrons, basis, xgrid, ygrid, Hcore, two_body,
...
@inject_params
def slater_determinant_closed_shell(x_config, C_occ, n_basis_x, n_basis_y, params=None, normalize=True):
    """
    ...
    phi_vals = evaluate_basis_functions_torch_batch_2d(x_config, n_basis_x, n_basis_y, omega)
...
def evaluate_basis_functions_torch_batch_2d(x, n_basis_x, n_basis_y, omega):
    ...
    phi_x = evaluate_basis_functions_torch(x_coord, n_basis_x, omega)  # shape: (B, N, n_basis_x)
    phi_y = evaluate_basis_functions_torch(y_coord, n_basis_y, omega)  # shape: (B, N, n_basis_y)
    
    # Compute the outer product of phi_x and phi_y for each electron.
    product = phi_x.unsqueeze(-1) * phi_y.unsqueeze(-2)  # shape: (B, N, n_basis_x, n_basis_y)
