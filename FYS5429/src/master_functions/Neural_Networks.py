...
@inject_params
def train_model(V, E, f_net, optimizer, C_occ, std=2.5, factor=0.1, params=None, print_e=50):
    """
    ...
    for epoch in range(n_epochs):
        ...
        optimizer.step()
        if epoch % print_e == 0:
            print(f"Epoch {epoch:05d}: PDE Loss = {loss_pde.item():.3e},Norm = {norm.item():.3e},  Variance = {variance.item():.3e}")
        del loss, variance, loss_pde, residual, H_psi, V_total, V_int, V_harmonic, psi, laplacian
